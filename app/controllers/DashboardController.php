...
class DashboardController
{
    ...
    /**
     * Réinitialise les besoins, dons et attributions aux données initiales
     */
    public function resetData()
    {
        ...
        try {
            ...
            // Supprimer toutes les données dans l'ordre (attributions d'abord à cause des FK)
            $attributionModel->deleteAll(); // Note: il y a un typo dans le modèle (deletteAll au lieu de deleteAll)
            $besoinModel->deleteAll();
            $achatModel->deleteAll();
            $donModel->deleteAll();


            // Réinitialiser les auto-increments
            $db->exec('ALTER TABLE bngrc_attribution AUTO_INCREMENT = 1');
            $db->exec('ALTER TABLE bngrc_besoin AUTO_INCREMENT = 1');
            $db->exec('ALTER TABLE bngrc_don AUTO_INCREMENT = 1');
            $db->exec('ALTER TABLE bngrc_achat AUTO_INCREMENT = 1');

            // Insérer les données initiales
            $besoinModel->initialiserBesoins();
            $donModel->initialiserDons();
